...
def alpha_beta_player(game, state):
    print("in the bot player")
    print("THIS IS THE RESULT FROM THE ALPHA BETA SEARCH")
    result = alpha_beta_search(state,game)
    print(result)
...
class TicTacToe(Game):
    ...
    def actions(self, state):
        print("IN THE ACTION FUNCTION")
        print("these are the list of actions")
        print(state.moves)
    ...
    def result(self, state, move):
        print("RESULT FUCNTION STATE -> BEFORE")
        ...
        moves.remove(move)
        print("RESULT FUNCTION STATE -> AFTER")
        return GameState(to_move=('0' if state.to_move == 'X' else 'X'),
                         utility=self.compute_utility(board, move, state.to_move),
                         board=board, moves=moves)
    def utility(self, state, player):
        print("IN THE UTILITY FUNCTION")
        """Return the value to player; 1 for win, -1 for loss, 0 otherwise."""
        return state.utility if player == 'X' else -state.utility

    def terminal_test(self, state):
        print("TERMINAL TEST FUNCTION")
